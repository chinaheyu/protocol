...
int protocol_calculate_frame_size(uint16_t data_size)
{
    if (data_size > PROTOCOL_DATA_MAX_SIZE)
    {
...
int protocol_pack_data_to_buffer(uint16_t cmd_id, const uint8_t *data, uint16_t len, uint8_t *buffer)
{
    int frame_size = protocol_calculate_frame_size(len);

    if (frame_size == PROTOCOL_ERROR) {
        return PROTOCOL_ERROR;
    }
    ...
    if(len > 0)
    {
        memcpy(p_header->p_data, data, len);
...
void protocol_initialize_unpack_object(protocol_unpack_object_t * unpack_obj)
{
    unpack_obj->unpack_step = STEP_HEADER_SOF;
    unpack_obj->index = 0;

#if PROTOCOL_USE_DYNAMIC_MEMORY == PROTOCOL_TRUE
    unpack_obj->protocol_packet = (uint8_t*)malloc(PROTOCOL_FRAME_MAX_SIZE);
#endif
}

int protocol_unpack_byte(protocol_unpack_object_t * unpack_obj, uint8_t byte)
{
